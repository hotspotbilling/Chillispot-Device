...
class RadiusChillispot
{
    ...
    {
        return [
            'title' => 'Radius Chillispot Openwrt',
            ...
            'url' => [
                ...
                'Telegram' => 'https://t.me/arnetadotid',
    ...
    { {
            global $radius_pass;
            $r = $this->getTableCustomer()->where_equal('username', $username)->whereEqual('attribute', 'Auth-Type')->findOne();
            if ($r) {
        ...
        if ($this->customerUpsert($customer, $plan)) {
            ...
            if ($plan['type'] == 'Hotspot' && $plan['typebp'] == "Limited") {
                if ($plan['limit_type'] == "Time_Limit") {
                    ...
                    $this->upsertCustomer($customer['username'], 'Access-Period', $timelimit);
    ...
    public function customerUpsert($customer, $plan) //Update or Insert customer plan
    {
        if ($plan['type'] == 'PPPOE') {
            ...
            $this->upsertCustomer($customer['username'], 'Auth-Type',  'Accept');
        }
